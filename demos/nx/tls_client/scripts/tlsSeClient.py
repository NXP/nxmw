...
if __name__ == '__main__':
    ...
    OPENSSL_CONF_SE="openssl11_sss_nx.cnf"

    if sys.platform.startswith("win"):
        library_name = "sss_engine.dll"
        openssl = os.path.join(cur_dir, '..', '..', '..', '..', 'ext', 'openssl', 'bin', 'openssl.exe')
        openssl_engine = os.path.join(cur_dir, '..', '..', '..', '..', 'plugin', 'openssl', 'bin', library_name)
    else:
        openssl = 'openssl'
    ...
    cmd=f"{openssl} s_client -connect {ip_addr}:8080 -engine {openssl_engine} -{TLS_OPTION} -CAfile {rootca_cer} -cert {client_cer} -key {client_key_ref} {sel_cipher} {groups} -debug -msg"
    log.info(cmd)
