...
static sss_status_t sdm_verify_mac_data(ex_sss_boot_ctx_t *pCtx,
    uint8_t *dnyFileData,
    size_t dnyFileDataLen,
    uint8_t *macData,
    size_t macDataLen,
    sss_object_t *keyMacObj);
    ...
    size_t *plainFileDataLen)
{
    ...
    // Enc data in Hex
    LOG_MAU8_D("encData in HEX", encData, encDataLen);
    ...
    sss_object_t *keyMacObj)
{
    ...
    sss_mac_t macCtx                                 = {0};
    uint8_t macHexTruncData[EX_SSS_SDM_SDMMACLength] = {0};
    size_t macHexTruncDataLen                        = sizeof(macHexTruncData);
    uint8_t macHexData[EX_SSS_SDM_SDMMACLength]      = {0};
    size_t macHexDataLen                             = sizeof(macHexData);
    uint8_t newTruncData[EX_SSS_SDM_SDMMACLength]    = {0};
    size_t newTruncDataLen                           = sizeof(newTruncData);
    uint8_t j                                        = 1;
    uint8_t i                                        = 0;

    if ((pCtx == NULL) || (macData == NULL) || (dnyFileData == NULL)) {
        LOG_E("Invalid input parameter");
        goto cleanup;
    }

    ENSURE_OR_GO_CLEANUP(keyMacObj != NULL);

    // Mac in Hex. Comes from read out data.
    ret = sdm_ascii_to_hex(macData, macDataLen, macHexTruncData, &macHexTruncDataLen);
    ENSURE_OR_GO_CLEANUP(0 == ret);

    status = sss_mac_context_init(&macCtx, &pCtx->host_session, keyMacObj, algorithm, mode);
    ENSURE_OR_GO_CLEANUP(status == kStatus_SSS_Success);

    status = sss_mac_one_go(
        &macCtx, dnyFileData, dnyFileDataLen, &macHexData[0], &macHexDataLen); //need to check macToAdd and maclen
    ENSURE_OR_GO_CLEANUP(status == kStatus_SSS_Success);

    // Trunc calculated MAC value
    if (macHexDataLen != EX_SSS_SDM_SDMMACLength) {
        LOG_E("Invalid mac value");
        status = kStatus_SSS_Fail;
        goto cleanup;
    }

    while (j < EX_SSS_SDM_SDMMACLength) {
        newTruncData[i++] = macHexData[j];
        j += 2;
    }
    newTruncDataLen = i;
    if ((macHexTruncDataLen == newTruncDataLen) && (memcmp(&macHexTruncData, newTruncData, macHexTruncDataLen) == 0)) {
        LOG_I("MAC verification passed");
...
sss_status_t ex_sss_entry(ex_sss_boot_ctx_t *pCtx)
{
    ...
    LOG_I("Verify SDM Mac @0x%x(Length 0x%x) with data @0x%x(Length 0x%x))",
        EX_SSS_SDM_SDMMACOffset,
        EX_SSS_SDM_SDMMACLength,
        EX_SSS_SDM_SDMMACInputOffset,
        EX_SSS_SDM_SDMMACOffset - EX_SSS_SDM_SDMMACInputOffset);
    status = sdm_verify_mac_data(pCtx,
