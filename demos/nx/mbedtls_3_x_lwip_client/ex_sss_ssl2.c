...
#else /* Embedded Platforms */
#include "ex_sss_ssl2.h"
...
static ex_sss_boot_ctx_t gmbedtls_3x_lwip_client_boot_ctx;

#define EX_SSS_BOOT_PCONTEXT (&gmbedtls_3x_lwip_client_boot_ctx)

#if defined(_MSC_VER) || defined(__linux__)
#define EX_SSS_BOOT_EXPOSE_ARGC_ARGV 1
#else
#define EX_SSS_BOOT_EXPOSE_ARGC_ARGV 0
#define EX_SSS_BOOT_RTOS_STACK_SIZE 10500
#endif /* defined(_MSC_VER) || defined(__linux__) */
/* ************************************************************************** */
...
sss_status_t ex_sss_entry(ex_sss_boot_ctx_t *pCtx)
{
    ...
    sss_status_t status =
        sss_host_session_open(&pCtx->host_session, kType_SSS_mbedTLS, 0, kSSS_ConnectionType_Plain, NULL);
...
usage:
    ...
    if (strlen(opt.ca_path)) {
        ...
        ret = mbedtls_x509_crt_parse(&cacert, (const unsigned char *)tls_rootca_file, tls_rootca_file_len);
    } else
#endif
    {
...
#else /* Bare-Metal OS */
    if (strlen(opt.crt_file)) {
        ret = mbedtls_x509_crt_parse(&clicert, (const unsigned char *)tls_client_file, tls_client_file_len);
    } else
#endif
...
#else /* Bare-Metal OS */
    if (strlen(opt.key_file)) {
        ret = mbedtls_pk_parse_key(&pkey, (const unsigned char *)tls_client_ref_key, tls_client_ref_key_len, NULL, 0, NULL, NULL);
    } else
#endif
