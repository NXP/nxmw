...
sss_status_t ex_sss_entry(ex_sss_boot_ctx_t *pCtx)
{
    ...
    // Reopen the session. (This will also make the SA wakeup from deep power down mode)
...
exit:
    if (status == kStatus_SSS_Success) {
        LOG_I("nx_deep_pwr_down Example Success !!!...");
    }
    else {
        LOG_I("nx_deep_pwr_down Example Failed !!!...");
    }
    return status;
}