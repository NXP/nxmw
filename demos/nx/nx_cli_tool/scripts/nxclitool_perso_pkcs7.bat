...
:: This script can be used to personalize the secure element with PKCS7 type certificates using the NX CLI Tool on Windows platform.
...
set CERT_FOLDER_PATH=..\..\..\..\binaries\configuration\cert_depth3_PKCS7_rev1\cert_and_key\nist_p
...
%TOOL_PATH%\nxclitool certrepo-load-key -keytype rootca -keyid %KEY_ID% -curve %CURVE% -certtype pkcs7 -in %CERT_FOLDER_PATH%\host_root_certificate.der
%TOOL_PATH%\nxclitool certrepo-load-key -keytype leaf -keyid %KEY_ID% -curve %CURVE% -certtype pkcs7 -in %CERT_FOLDER_PATH%\device_leaf_keypair.der
...
%TOOL_PATH%\nxclitool certrepo-load-mapping -repoid %REPO_ID% -certlevel leaf -kcomm na -in %CERT_FOLDER_PATH%\host_leaf_cert_mapping.bin
%TOOL_PATH%\nxclitool certrepo-load-mapping -repoid %REPO_ID% -certlevel p1 -kcomm na -in %CERT_FOLDER_PATH%\host_p1_cert_mapping.bin
%TOOL_PATH%\nxclitool certrepo-load-mapping -repoid %REPO_ID% -certlevel p2 -kcomm na -in %CERT_FOLDER_PATH%\host_p2_cert_mapping.bin
