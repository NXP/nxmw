...
#if (defined(SSS_HAVE_HOST_FRDMMCXN947) && (SSS_HAVE_HOST_FRDMMCXN947))
 ...
 */
static void demoPubCallbackHandler(
    MQTTContext_t *pxMqttContext, MQTTPacketInfo_t *pxPacketInfo, MQTTDeserializedInfo_t *pxDeserializedInfo);

/*******************************************************************************
 * Functions
 ******************************************************************************/

static BaseType_t xPkcs11GenerateRandomNumber(uint8_t *pusRandomNumBuffer, size_t xBufferLength)
{
    ...
    if ((pusRandomNumBuffer == NULL) || (xBufferLength == 0U)) {
        LogError(
            ("Cannot generate random number. Invalid parameters passed. "
             "buffer=%p,bufferLen=%lu",
                pusRandomNumBuffer,
                xBufferLength));
        xStatus = pdFAIL;
    }
    ...
    if (xStatus == pdPASS) {
        if (pxFunctionList->C_GenerateRandom(xSession, pusRandomNumBuffer, xBufferLength) != CKR_OK) {
            xStatus = pdFAIL;
            LogError(
                ("Failed to generate random number. "
...
static TlsTransportStatus_t prvConnectToServerWithBackoffRetries(NetworkContext_t *pxNetworkContext)
{
    ...
    do {
        ...
        if (xNetworkStatus != TLS_TRANSPORT_SUCCESS) {
            LogWarn((
                "Connection to the broker failed. Attempting connection retry after backoff delay %d", xNetworkStatus));
...
static void vCleanupOutgoingPublishAt(uint8_t ucIndex)
{
    ...
    (void)memset(&(outgoingPublishPackets[ucIndex]), 0x00, sizeof(PublishPackets_t));
}
...
void vHandleOtherIncomingPacket(MQTTPacketInfo_t *pxPacketInfo, uint16_t usPacketIdentifier)
{
...
static BaseType_t handlePublishResend(MQTTContext_t *pxMqttContext)
{
    ...
    for (ucIndex = 0U; ucIndex < MAX_OUTGOING_PUBLISHES; ucIndex++) {
        if (outgoingPublishPackets[ucIndex].packetId != MQTT_PACKET_ID_INVALID) {
            ...
            LogInfo(("Sending duplicate PUBLISH with packet id %u.", outgoingPublishPackets[ucIndex].packetId));
            eMqttStatus = MQTT_Publish(
                pxMqttContext, &outgoingPublishPackets[ucIndex].pubInfo, outgoingPublishPackets[ucIndex].packetId);

            if (eMqttStatus != MQTTSuccess) {
                LogError(
                    ("Sending duplicate PUBLISH for packet id %u "
    ...
    MQTTEventCallback_t eventCallback)
{
    ...
    if (prvConnectToServerWithBackoffRetries(pxNetworkContext) != TLS_TRANSPORT_SUCCESS) {
        ...
        eMqttStatus = MQTT_Init(pxMqttContext, &xTransport, prvGetTimeMs, eventCallback, pxNetworkBuffer);

        if (eMqttStatus != MQTTSuccess) {
            xReturnStatus = pdFAIL;
            LogError(("MQTT init failed with status %s.", MQTT_Status_strerror(eMqttStatus)));
        }
        else {
            ...
            eMqttStatus =
                MQTT_Connect(pxMqttContext, &xConnectInfo, NULL, mqttexampleCONNACK_RECV_TIMEOUT_MS, &sessionPresent);

            if (eMqttStatus != MQTTSuccess) {
                xReturnStatus = pdFAIL;
                LogError(("Connection with MQTT broker failed with status %s.", MQTT_Status_strerror(eMqttStatus)));
            }
            else {
        ...
        if (xReturnStatus == pdPASS) {
            ...
            if (sessionPresent == true) {
                LogInfo(
                    ("An MQTT session with broker is re-established. "
...
BaseType_t DisconnectMqttSession(MQTTContext_t *pxMqttContext, NetworkContext_t *pxNetworkContext)
{
    MQTTStatus_t eMqttStatus = MQTTSuccess;
    BaseType_t xReturnStatus = pdPASS;
    ...
    if (mqttSessionEstablished == true) {
        ...
        if (eMqttStatus != MQTTSuccess) {
            LogError(("Sending MQTT DISCONNECT failed with status=%s.", MQTT_Status_strerror(eMqttStatus)));
            xReturnStatus = pdFAIL;
        }
    }
...
BaseType_t UnsubscribeFromTopic(MQTTContext_t *pxMqttContext, const char *pcTopicFilter, uint16_t usTopicFilterLength)
{
    ...
    if (eMqttStatus != MQTTSuccess) {
        LogError(("Failed to send UNSUBSCRIBE packet to broker with error = %s.", MQTT_Status_strerror(eMqttStatus)));
        xReturnStatus = pdFAIL;
    }
    // else
    // {
    ...
    size_t payloadLength)
{
    ...
    if (xReturnStatus == pdFAIL) {
        ...
        if (eMqttStatus != MQTTSuccess) {
            LogError(("Failed to send PUBLISH packet to broker with error = %s.", MQTT_Status_strerror(eMqttStatus)));
...
BaseType_t ProcessLoop(MQTTContext_t *pxMqttContext, uint32_t ulTimeoutMs)
{
    BaseType_t xReturnStatus = pdFAIL;
    MQTTStatus_t eMqttStatus = MQTTSuccess;
...
static uint32_t prvGetTimeMs(void)
{
    ...
    assert(ulTimeMs >= ulGlobalEntryTimeMs);
...
void demoPubCallbackHandler(
    MQTTContext_t *pxMqttContext, MQTTPacketInfo_t *pxPacketInfo, MQTTDeserializedInfo_t *pxDeserializedInfo)
{
    /* Received a published message */
    assert(pxDeserializedInfo->pPublishInfo->payloadLength <= UINT16_MAX);
    HandleReceivedMsg(
        (char *)pxDeserializedInfo->pPublishInfo->pPayload, pxDeserializedInfo->pPublishInfo->payloadLength);
}
...
void awsPubSub(const U8 ax_uid[MODULE_UNIQUE_ID_LEN])
{
    ...
    MQTTFixedBuffer_t xNetworkBuffer = {.pBuffer = ucSharedBuffer, .size = democonfigNETWORK_BUFFER_SIZE};

    xResult = EstablishMqttSession(&xMqttContext, &xNetworkContext, &xNetworkBuffer, prvEventCallback);
    ...
    xResult = SubscribeToTopic(&xMqttContext, SUB_TOPIC, strlen(SUB_TOPIC));
    if (xResult != pdPASS) {
    ...
    xResult = PublishToTopic(&xMqttContext, PUB_TOPIC, strlen(PUB_TOPIC), cPayload, strlen(cPayload));
    if (xResult != pdPASS) {
