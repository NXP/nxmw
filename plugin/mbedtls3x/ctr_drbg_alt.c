...
static sss_key_store_t *g_rng_keystore = NULL;
    ...
    for (j = 0; j < MBEDTLS_CTR_DRBG_SEEDLEN; j += MBEDTLS_CTR_DRBG_BLOCKSIZE) {
        ...
        while (use_len > 0) {
            ...
            status = psa_cipher_update(
                &psa_ctx.operation, chain, MBEDTLS_CTR_DRBG_BLOCKSIZE, chain, MBEDTLS_CTR_DRBG_BLOCKSIZE, &tmp_len);
            if (status != PSA_SUCCESS) {
                ret = psa_generic_status_to_mbedtls(status);
                goto exit;
            }
...
#if defined(MBEDTLS_CTR_DRBG_USE_PSA_CRYPTO)
        status = psa_cipher_update(
            &psa_ctx.operation, iv, MBEDTLS_CTR_DRBG_BLOCKSIZE, iv, MBEDTLS_CTR_DRBG_BLOCKSIZE, &tmp_len);
        if (status != PSA_SUCCESS) {
            ret = psa_generic_status_to_mbedtls(status);
            goto exit;
        }
...
#if defined(MBEDTLS_CTR_DRBG_USE_PSA_CRYPTO)
        status = psa_cipher_update(
            &ctx->psa_ctx.operation, ctx->counter, sizeof(ctx->counter), p, MBEDTLS_CTR_DRBG_BLOCKSIZE, &tmp_len);
        if (status != PSA_SUCCESS) {
            ret = psa_generic_status_to_mbedtls(status);
            goto exit;
        }
 ...
 */
static int mbedtls_ctr_drbg_reseed_internal(
    mbedtls_ctr_drbg_context *ctx, const unsigned char *additional, size_t len, size_t nonce_len)
{
    ...
    void *p_rng, unsigned char *output, size_t output_len, const unsigned char *additional, size_t add_len)
{
    ...
    while (output_len > 0) {
        ...
        status = psa_cipher_update(&ctx->psa_ctx.operation,
            ctx->counter,
            sizeof(ctx->counter),
            locals.tmp,
...
#else
        if ((ret = mbedtls_aes_crypt_ecb(&ctx->aes_ctx, MBEDTLS_AES_ENCRYPT, ctx->counter, locals.tmp)) != 0) {
            goto exit;
        }
#endif
...
int mbedtls_ctr_drbg_random_nx(void *p_rng, unsigned char *output, size_t output_len)
{
    ...
    ENSURE_OR_GO_EXIT(p_rng != NULL);
    (void)p_rng;

    status = sss_rng_context_init(&rng_ctx, g_rng_keystore->session);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);

    LOG_I("Random number generation using NX Secure Authenticator");

    status = sss_rng_get_random(&rng_ctx, output, output_len);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
...
int mbedtls_ctr_drbg_random(void *p_rng, unsigned char *output, size_t output_len)
{
    if (g_rng_keystore != NULL) {
        return mbedtls_ctr_drbg_random_nx(p_rng, output, output_len);
    }
    else {
        int ret                       = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
        mbedtls_ctr_drbg_context *ctx = (mbedtls_ctr_drbg_context *)p_rng;
...
static const unsigned char entropy_source_pr[] = {0x04,
    0xd9,
    0x49,
    0xa6,
    0xdc,
...
static const unsigned char entropy_source_nopr[] = {0x07,
    0x0d,
    0x59,
    0x63,
    0x98,
...
static const unsigned char pers_pr[] = {
    0xbf, 0xa4, 0x9a, 0x8f, 0x7b, 0xd8, 0xb1, 0x7a, 0x9d, 0xfa, 0x45, 0xed, 0x21, 0x52, 0xb3, 0xad};

static const unsigned char pers_nopr[] = {
    0x4e, 0x61, 0x79, 0xd4, 0xc2, 0x72, 0xa1, 0x4c, 0xf1, 0x3d, 0xf6, 0x5e, 0xa3, 0xa6, 0xe5, 0x0f};

static const unsigned char result_pr[] = {0xc9,
    0x0a,
    0xaf,
    0x85,
    0x89,
...
static const unsigned char result_nopr[] = {0x31,
    0xc9,
    0x91,
    0x09,
    0xf8,
...
static const unsigned char entropy_source_nopr[] = {0x4c,
    0xfb,
    0x21,
    0x86,
    0x73,
...
static const unsigned char pers_pr[] = {0x5a,
    0x70,
    0x95,
    0xe9,
    0x81,
...
static const unsigned char pers_nopr[] = {0x88,
    0xee,
    0xb8,
    0xe0,
    0xe8,
...
static const unsigned char result_pr[] = {0xce,
    0x2f,
    0xdb,
    0xb6,
    0xd9,
...
static const unsigned char result_nopr[] = {0xa5,
    0x51,
    0x80,
    0xa1,
    0x90,
...
int mbedtls_ctr_drbg_self_test(int verbose)
{
    ...
    CHK(mbedtls_ctr_drbg_seed(
        &ctx, ctr_drbg_self_test_entropy, (void *)entropy_source_pr, pers_pr, MBEDTLS_CTR_DRBG_KEYSIZE));
    ...
    mbedtls_ctr_drbg_set_nonce_len(&ctx, MBEDTLS_CTR_DRBG_KEYSIZE / 2);
    CHK(mbedtls_ctr_drbg_seed(
        &ctx, ctr_drbg_self_test_entropy, (void *)entropy_source_nopr, pers_nopr, MBEDTLS_CTR_DRBG_KEYSIZE));
