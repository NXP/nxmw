...
#if defined(MBEDTLS_AES_USE_HARDWARE_ONLY)
#error \
    "MBEDTLS_AES_USE_HARDWARE_ONLY cannot be defined when " \
...
static sss_key_store_t *g_aes_keystore = NULL;
 ...
 * Forward S-box
 */
MBEDTLS_MAYBE_UNUSED static const unsigned char FSb[256] = {0x63,
    0x7C,
    0x77,
    0x7B,
    0xF2,
 ...
 * Reverse S-box
 */
MBEDTLS_MAYBE_UNUSED static const unsigned char RSb[256] = {0x52,
    0x09,
    0x6A,
    0xD5,
    0x30,
 ...
 * Round constants
 */
MBEDTLS_MAYBE_UNUSED static const uint32_t round_constants[10] = {0x00000001,
    0x00000002,
    0x00000004,
    0x00000008,
    0x00000010,
 ...
 */
#if (defined(MBEDTLS_VIA_PADLOCK_HAVE_CODE)) || (defined(MBEDTLS_AESNI_C) && MBEDTLS_AESNI_HAVE_CODE == 2)
    ...
    case 14:
        ...
        for (unsigned int i = 0; i < 7; i++, RK += 8) {
            ...
            RK[12] = RK[4] ^ ((uint32_t)FSb[MBEDTLS_BYTE_0(RK[11])]) ^ ((uint32_t)FSb[MBEDTLS_BYTE_1(RK[11])] << 8) ^
                     ((uint32_t)FSb[MBEDTLS_BYTE_2(RK[11])] << 16) ^ ((uint32_t)FSb[MBEDTLS_BYTE_3(RK[11])] << 24);
...
#if defined(MBEDTLS_AESCE_HAVE_CODE)
    if (MBEDTLS_AESCE_HAS_SUPPORT()) {
        mbedtls_aesce_inverse_key((unsigned char *)RK, (const unsigned char *)(cty.buf + cty.rk_offset), ctx->nr);
        goto exit;
    }
#endif
...
int mbedtls_aes_crypt_ecb_nx(mbedtls_aes_context *ctx, int mode, const unsigned char *input, unsigned char *output)
{
    ...
    uint8_t aes_key_id           = kSE_CryptoAESKey_TB_SLOTNUM_0;
    size_t aes_key_len           = 0;
    uint32_t *RK                 = ctx->buf + ctx->rk_offset;
    uint8_t aes_key_data[32]     = {0};

    sss_nx_key_store_t *nx_aes_keystore = (sss_nx_key_store_t *)g_aes_keystore;
    sss_nx_session_t *pSession          = (sss_nx_session_t *)(nx_aes_keystore->session);
    ...
    if (ctx->nr == 10) {
        ...
        LOG_E("Invalid AES key length");
        return 1;
    }

    if (mode == MBEDTLS_AES_ENCRYPT) {
        primitive = Nx_AES_Primitive_ECB_Encrypt;
        memcpy(aes_key_data, (uint8_t *)RK, aes_key_len);
        LOG_I("AES ECB encrypt using NX (By storing key at slot # %02X)", aes_key_id);
    }
    else if (mode == MBEDTLS_AES_DECRYPT) {
        primitive = Nx_AES_Primitive_ECB_Decrypt;
        RK        = ctx->buf + ctx->rk_offset + ctx->nr * 4;
        memcpy(aes_key_data, (uint8_t *)RK, aes_key_len);
        LOG_I("AES ECB decrypt using NX (By storing key at slot # %02X)", aes_key_id);
    }

    sm_status = nx_CryptoRequest_Write_Internal_Buffer(&pSession->s_ctx, aes_key_id, &aes_key_data[0], aes_key_len);
    if (SM_OK != sm_status) {
        LOG_E("nx_CryptoRequest_Write_Internal_Buffer is failed");
        return 1;
    }

    sm_status = nx_CryptoRequest_AES_CBC_ECB_Oneshot(&pSession->s_ctx,
        primitive,
        aes_key_id,
        (uint8_t)aes_key_len,
        ...
        &output[0]);
    if (sm_status != SM_OK) {
        LOG_E("nx_CryptoRequest_AES_CBC_ECB_Oneshot is failed");
        return 1;
    }

    return 0;
}
#endif //defined(MBEDTLS_AES_DECRYPT_ALT) || defined(MBEDTLS_AES_ENCRYPT_ALT)
...
int mbedtls_internal_aes_encrypt(mbedtls_aes_context *ctx, const unsigned char input[16], unsigned char output[16])
{
    ...
    if ((g_aes_keystore != NULL) && (ctx->nr == 10 || ctx->nr == 14)) {
        return mbedtls_aes_crypt_ecb_nx(ctx, MBEDTLS_AES_ENCRYPT, input, output);
    }
    else {
        int i;
        uint32_t *RK = ctx->buf + ctx->rk_offset;
        ...
        LOG_I("mbedtls_internal_aes_encrypt - rollback to software implementation");
...
int mbedtls_internal_aes_decrypt(mbedtls_aes_context *ctx, const unsigned char input[16], unsigned char output[16])
{
    ...
    if ((g_aes_keystore != NULL) && (ctx->nr == 10 || ctx->nr == 14)) {
        return mbedtls_aes_crypt_ecb_nx(ctx, MBEDTLS_AES_DECRYPT, input, output);
    }
    else {
        int i;
        uint32_t *RK = ctx->buf + ctx->rk_offset;
        ...
        LOG_I("mbedtls_internal_aes_decrypt - rollback to software implementation");
...
static const unsigned char aes_test_cfb128_pt[64] = {0x6B,
    0xC1,
    0xBE,
    0xE2,
    0x2E,
...
static const unsigned char aes_test_cfb128_ct[][64] = {{0x3B,
                                                           0x3F,
                                                           0xD9,
                                                           0x2E,
                                                           0xB7,
...
static const unsigned char aes_test_ofb_pt[64] = {0x6B,
    0xC1,
    0xBE,
    0xE2,
    0x2E,
...
static const unsigned char aes_test_ofb_ct[][64] = {{0x3B,
                                                        0x3F,
                                                        0xD9,
                                                        0x2E,
                                                        0xB7,
