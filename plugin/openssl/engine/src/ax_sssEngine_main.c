...
#else
        || !setup_ec_key_method() || !ENGINE_set_EC(e, EmbSe_EC) || !setup_rsa_key_method() ||
...
void EmbSe_Print(int flag, const char *format, ...)
{
    ...
    if (active == 1) {
        ...
        if (vsnprintf((char *)buffer, EMBSE_MAX_PRINT_BUF_SIZE, (char const *)format, vArgs) < 0) {
