...
/**
 * OpenSSL Provider for NXP Embedded Secure Authenticator over SSS API's
 *
...
static const OSSL_ALGORITHM sss_rands[] = {
    {"CTR-DRBG", "provider=nxp_prov,nxp_prov.rand=yes", sss_rand_functions}, {NULL, NULL, NULL}};

extern OSSL_DISPATCH sss_ec_keymgmt_functions[];

static const OSSL_ALGORITHM sss_keymgmts[] = {
    {"ec:id-ecpublickey", "provider=nxp_prov,nxp_prov.keymgmt=yes,nxp_prov.keymgmt.ec=yes", sss_ec_keymgmt_functions},
    {NULL, NULL, NULL}};

extern const OSSL_DISPATCH sss_ecdsa_signature_functions[];

static const OSSL_ALGORITHM sss_signatures[] = {
    {"ECDSA", "provider=nxp_prov,nxp_prov.signature=yes,nxp_prov.signature.ecdsa=yes", sss_ecdsa_signature_functions},
    {NULL, NULL, NULL}};

extern const OSSL_DISPATCH sss_ecdh_keyexch_functions[];

static const OSSL_ALGORITHM sss_keyexchs[] = {
    {"ECDH", "provider=nxp_prov,nxp_prov.keyexch=yes", sss_ecdh_keyexch_functions}, {NULL, NULL, NULL}};
...
static const OSSL_ALGORITHM sss_store[] = {
    {"nxp", "provider=nxp_prov,nxp_prov.store=yes,nxp_prov.store.nxp=yes", sss_store_object_functions},
    {"file", "provider=nxp_prov,nxp_prov.store=yes,nxp_prov.store.file=yes", sss_file_store_object_functions},
    {NULL, NULL, NULL}};
...
static const OSSL_ALGORITHM *sss_query_operation(void *provctx, int operation_id, int *no_cache)
{
    *no_cache = 0;
    (void)(provctx);
    switch (operation_id) {
#ifndef SSS_PROV_DISABLE_NX_RNG
    case OSSL_OP_RAND:
        return sss_rands;
#endif
    ...
    const OSSL_CORE_HANDLE *handle, const OSSL_DISPATCH *in, const OSSL_DISPATCH **out, void **provctx)
{
    ...
    // Load default provider to use during symm/sigma Authentication
