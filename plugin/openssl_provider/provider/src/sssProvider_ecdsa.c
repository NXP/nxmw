    ...
    if (pEcdsaCtx->pStoreObjCtx->object.keyId != 0) {
        ...
        if (sig == NULL) {
            *siglen = (((pEcdsaCtx->pStoreObjCtx->key_len) * 2) + 8);
            ...
            sssProv_Print(LOG_FLOW_ON, "Performing ECDSA sign using Secure Authenticator \n");
            sssProv_Print(
                LOG_DBG_ON, "(Using key id 0x%X from Secure Authenticator) \n", pEcdsaCtx->pStoreObjCtx->object.keyId);
        ...
        if (sig == NULL) {
            ...
            sssProv_Print(LOG_FLOW_ON,
                "Not a key in Secure Authenticator. Performing ECDSA sign operation using host software \n");
...
static int sss_ecdsa_signature_digest_init(void *ctx, const char *mdname, void *provkey, const OSSL_PARAM params[])
{
    ...
    if (mdname == NULL) {
        ...
        if ((0 == SSS_CMP_STR(mdname, "sha256")) || (0 == SSS_CMP_STR(mdname, "SHA256")) ||
    ...
    void *ctx, const unsigned char *sig, size_t siglen, const unsigned char *tbs, size_t tbslen)
{
    ...
    if (pEcdsaCtx->pStoreObjCtx->object.keyId != 0) {
        ...
        sssProv_Print(LOG_FLOW_ON, "Performing ECDSA verify using Secure Authenticator \n");
        sssProv_Print(
            LOG_DBG_ON, "(Using key id 0x%X from Secure Authenticator) \n", pEcdsaCtx->pStoreObjCtx->object.keyId);
        ...
        sssProv_Print(
            LOG_FLOW_ON, "Not a key in Secure Authenticator. Performing ECDSA verify operation using host software \n");
    ...
    void *ctx, unsigned char *sig, size_t sigsize, const unsigned char *data, size_t datalen)
{
    ...
    if (pEcdsaCtx->pStoreObjCtx->object.keyId != 0) {
        ...
        sssProv_Print(LOG_FLOW_ON, "Performing ECDSA verify using NX SA \n");
        sssProv_Print(LOG_DBG_ON, "(Using key id 0x%X from NX SA) \n", pEcdsaCtx->pStoreObjCtx->object.keyId);
        status =
        ...
        sssProv_Print(LOG_FLOW_ON,
            "Not a key in secure authenticator. Performing ECDSA "
...
static int sss_ecdsa_signature_get_ctx_params(void *ctx, OSSL_PARAM *params)
{
    ...
    if (pEcdsaCtx->sha_algorithm == kAlgorithm_SSS_SHA256) {
