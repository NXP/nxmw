...
static bool_t phNxpEseProto7816_CheckCRC(uint32_t data_len, uint8_t *p_data)
{
    ...
    recv_crc = (uint16_t)(p_data[data_len - 2] << 8 | p_data[data_len - 1]); //combine 2 byte CRC
...
static bool_t phNxpEseProto7816_SendSFrame(void *conn_ctx, sFrameInfo_t sFrameData)
{
    ...
    /* frame the packet */
    p_framebuff[PH_PROPTO_7816_NAD_OFFSET] = 0x21;     /* NAD Byte */
...
static bool_t phNxpEseProto7816_sendRframe(void *conn_ctx, rFrameTypes_t rFrameType)
{
    bool_t status = FALSE;
#if defined(T1oI2C_GP1_0) || defined(SSS_HAVE_SMCOM_T1OI2C_GP1_0) && (SSS_HAVE_SMCOM_T1OI2C_GP1_0)
    uint8_t recv_ack[6] = {0x21, 0x80, 0x00, 0x00, 0x00, 0x00};
...
static bool_t phNxpEseProro7816_SaveRxframeData(uint8_t *p_data, uint32_t data_len)
{
    ...
    LOG_D("Data[0]=0x%x len=%ld Data[%ld]=0x%x Data[%ld]=0x%x",
        p_data[0],
        data_len,
        data_len - 1,
        p_data[data_len - 2],
...
#endif
    ENSURE_OR_GO_EXIT(p_data[frameOffset] <= (UINT8_MAX - frameOffset));
...
bool_t phNxpEseProto7816_Transceive(void *conn_ctx, phNxpEse_data *pCmd, phNxpEse_data *pRsp)
{
    ...
    pRsp->len                                              = (uint32_t)pRx_EseCntx->responseBytesRcvd;
    phNxpEseProto7816_3_Var.phNxpEseProto7816_CurrentState = PH_NXP_ESE_PROTO_7816_IDLE;
    return status;
}
 ...
 *
 * Description      This function is used to send release req command
 *
 * param[in]        void
 *
 * Returns          On success return TRUE or else FALSE.
